<?
	//
	//  manage_information_edit_module.php
	//  RCMS
	//
	...
	$strSQL = "SELECT * FROM db_user,db_userdata WHERE db_user.UserID = db_userdata.UserID AND db_user.UserID = '".$_POST['UserID']."' ";
...
<div>
	<div id="search">
		<input type="submit" class="submit" value="Save" OnClick="JavaScript:editAjax('<?=$objResult["UserID"];?>')"/>
		<input type="submit" class="submit" value="Cancel" OnClick="JavaScript:showBox('manage_information.php','<?=$objResult["UserID"];?>')"/>
	</div><!-- search -->
	<table>
		<tbody>
			<tr>
